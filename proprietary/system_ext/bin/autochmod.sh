...
function initcache(){
    ...
    if [ "${ALWAYSON_ENABLE}" == "true" ]; then
        traceTransferState "LAO enabled, return!!"
        return
    fi
...
function logcatcache(){
    ...
    if [[ "${panicenable}" = "${argtrue}" ]] || [[ x"${camerapanic}" = x"true" ]]; then
        ...
        /system/bin/logcat -f ${CACHE_PATH}/apps/android_boot.txt -r 10240 -n 5 -v threadtime
    fi
}
function radiocache(){
...
function kernelcacheforopm(){
  ...
  rm -rf ${DATA_DEBUGGING_PATH}/powermonitor_temp
...
function handle_m_commonLog(){
    ...
    traceTransferState "start logcatxxx..."
...
function initLogSizeAndNums() {
    ...
    if [ ${FreeSize} -ge ${GSIZE} ]; then
        ...
        if [ "${tmpRadio}" != "" ]; then
            ...
            traceTransferState "tmpRadioSize=${tmpRadioSize}; tmpRadioCount=${tmpRadioCount} radioSize=${radioSize} radioCount=${radioCount}"
            if [ ${radioCount} -ge ${tmpRadioCount} ]; then
                radioCount=${tmpRadioCount}
            fi
            traceTransferState "last radioCount=${radioCount}"
        fi
        ...
        if [ "${tmpEvent}" != "" ]; then
            ...
            traceTransferState "tmpEventSize=${tmpEventSize}; tmpEventCount=${tmpEventCount} eventSize=${eventSize} eventCount=${eventCount}"
            if [ ${eventCount} -ge ${tmpEventCount} ]; then
                eventCount=${tmpEventCount}
            fi
            traceTransferState "last eventCount=${eventCount}"
        fi
        ...
        if [ "${tmpTcpdump}" != "" ]; then
            ...
            traceTransferState "last tcpdumpCount=${tcpdumpCount}"
        else
            traceTransferState "tmpTcpdump is empty"
        fi
    else
        traceTransferState "free size is less than 2G"
        androidSize=20480
...
function dumpMedia() {
    ...
    mediaPath="${DATA_DEBUGGING_PATH}/SI_stop/media"
    mkdir -p ${mediaPath}
...
function dumpon(){
    ...
    if [ -d ${dump_sm8450_wlan_log_dir} ]; then
            engVersion=`cat /proc/oplusVersion/engVersion`
            echo version ${engVersion}
            setprop persist.sys.wifidumpVersion ${engVersion}
            if [ "${engVersion}" = "1" ] ; then
               echo 1 > ${dump_sm8450_wlan_log_dir}/recovery
               setprop persist.sys.wifidumptest 1
            else
               echo 0 > ${dump_sm8450_wlan_log_dir}/recovery
               setprop persist.sys.wifidumptest 0
            fi
    fi
}
...
function packReturnLog() {
    sleep 20
    COLLECT_LOG_PATH="${DATA_DEBUGGING_PATH}/wifi_log_temp"
    DCS_WIFI_LOG_PATH="${DATA_OPLUS_LOG_PATH}/DCS/de/network_logs/wifi_log"
    zip_name=`getprop sys.oplus.wifi.log.zipPath`
    ...
    sleep 5
    setprop sys.oplus.wifi.log.collectorOn "false"
}
#endif /* OPLUS_BUG_STABILITY */
...
function deleteLog() {
    ...
    rm -rf DUMP_SYSTEM_LOG

    #/sdcard/Android/data/com.oplus.logkit/files/Log/trigger
    rm -rf ${SDCARD_LOG_TRIGGER_PATH}

    #/data/debugging/wm/*
    rm -rf ${DATA_DEBUGGING_PATH}/wm/*
